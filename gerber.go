...
// For an example implementation, visit https://pkg.go.dev/github.com/fumin/gerber/svg .
...
func (p *commandProcessor) flash(lineIdx int) error {
	...
	case templateNameRectangle:
		p.pc.Rectangle(lineIdx, p.x, p.y, p.u(params[0]), p.u(params[1]), p.polarity)
	case templateNameObround:
		p.pc.Obround(lineIdx, p.x, p.y, p.u(params[0]), p.u(params[1]), p.polarity)
	default:
...
func (p *commandProcessor) flashUserDefinedTmpl(lineIdx int) error {
	...
	for i, primitive := range p.ap.Template.Primitives {
		...
		case lowerLeftLinePrimitive:
			...
			p.pc.Rectangle(lineIdx, p.x+p.u(pm.X+pm.Width/2), p.y+p.u(pm.Y+pm.Height/2), p.u(pm.Width), p.u(pm.Height), p.polarity)
		default:
